    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-background/90 backdrop-blur-sm border-b border-border/50">
      <div className="container mx-auto flex items-center justify-between h-20 px-6">
        <button onClick={() => scrollTo("hero")} className="flex items-center gap-3">
          <img src={logo} alt="Kendra Beauty Care" className="h-12 w-12 rounded-full object-cover" />
          <span className="font-heading text-xl tracking-elegant text-foreground hidden sm:inline">
            KENDRA
          </span>
        </button>
      ...
      {isOpen && (
        <div className="md:hidden bg-background border-t border-border/50 px-6 py-6 space-y-4">
          ...
          <button
            onClick={() => scrollTo("reserva")}
            className="w-full bg-primary text-primary-foreground px-6 py-3 text-xs tracking-elegant uppercase font-body rounded-sm"
          >
            Reservar Cita
          </button>
        </div>
